...
The manifest declares the scalable `/favicon.svg` (size `any`, purpose `any`) alongside raster `/icon-192.png` and `/icon-512.png` (purpose `any`) and a full-bleed `/icon-maskable-512.png` (purpose `maskable`); the [icon-set note](2026-08-19-web-app-icon-set.md) owns the mark and the raster pipeline, added once iOS Home Screen gave a concrete raster target.
...
The built-Web test parses the emitted manifest and pins the complete metadata object, including the human-visible name, compact name, icon set, root identity, launch boundary, and display mode, while also verifying that the production `index.html` retains the manifest link and the `apple-touch-icon` link. The `dsh-host-frontend-static` real Loader composition test serves a `.webmanifest` fixture and pins its `application/manifest+json` media type.
...
**Ship raster and maskable icon variants immediately.** Deferred at first because no installation target then required more than the scalable favicon; iOS Safari's blurry Home Screen bookmark later supplied that requirement, so the [icon-set note](2026-08-19-web-app-icon-set.md) shipped the raster and maskable variants as the additive manifest change this alternative anticipated.
