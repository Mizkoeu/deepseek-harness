...
- `llm.discoverModels` 把同一份草稿送过协议层。它的 `apiKey` 是可承载机密的第三个、也是最后一个载荷（另两个是 `settings.update`/`mutate` 与 `credentials.set`），且绝不被存储或回显。它确实会像其他承载机密的载荷一样随客户端外发信封同行，`subscribeEnvelopes()` 观察者看得到；把那个抽头脱敏是整个配置面的改动，不该由这一个方法独自决定。除密钥之外，把它归入连接守卫特权集还有第二个理由：它让宿主向调用方选定的 URL 发起 GET 并回报结果，这是任何不可信调用者都不该拥有的探测能力。该集合通过 `trustedHosts` 允许列表，因此已声明的可信授权可以到达它，而不可信或被重绑定的 Host 会被拒绝（见 [trusted-remote 特权操作](2026-08-19-trusted-remote-privileged-actions.md)）。每一种拒绝都折叠为 `model-discovery-failed`，其消息是适配器自己的文本，details 点名被询问的端点，绝不点名所提供的凭据。
