...
- The privileged configuration/credentials/native-action methods ride this same fence against `trustedHosts`, so a declared trusted authority reaches them; the personal trusted-device deployment that admits is scoped in [trusted-remote privileged actions](2026-08-19-trusted-remote-privileged-actions.md).
