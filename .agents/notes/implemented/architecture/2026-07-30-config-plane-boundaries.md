...
**Reading configuration is as privileged as writing it.** `settings.describe` and `credentials.describe` join the privileged method set that passes the connection trust fence, so the whole configuration plane is refused for any untrusted or rebound Host. The model catalog (`llm.providers`, `llm.models`) deliberately does not: it carries provider ids, display names, and model lists — no endpoints, no key state — and a LAN client's model picker needs it. The boundary is asserted over a real HTTP server rather than a hand-assembled request, because the `Host` header a browser actually sends is what decides it. This note's original enforcement pinned the privileged set to loopback; that caller restriction is partially superseded — the set now passes the same `trustedHosts` allowlist as every other `/api` call, so a declared trusted authority reaches it ([trusted-remote privileged actions](2026-08-19-trusted-remote-privileged-actions.md)). Which methods are privileged, and that reads count, is unchanged.
