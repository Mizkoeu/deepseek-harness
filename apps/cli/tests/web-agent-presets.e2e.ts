...
async function bootWeb(
  ...
  const patches: PatchOptions[] = [
    ...
    // The shipped web bundle defaults to the in-app `-browse` picker, which
    // provides `directoryPicker` without a webserver. This test disables the
    // webserver above, so the shipped default composes as-is with no picker swap.
