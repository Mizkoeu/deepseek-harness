...
` ```mermaid ` 围栏在定稿一遍上通过 `MermaidBlock` 渲染为内联图表：mermaid 及其 DOMPurify 净化器在首次使用时加载（惰性 `import()`，因此没有图表的会话不会给打包体积增加任何内容），SVG 在进入 DOM 之前经过双重净化（mermaid `securityLevel: 'strict'` 加上第二遍仅保留 SVG 词汇表与图表自身 `<style>` 的 DOMPurify），图表的明暗主题跟随文档配色。回复流式输出期间，该围栏保持为纯代码块；语法解析失败的图表回退为其源代码块。原理：[内联 mermaid 图表笔记](../../../.agents/notes/implemented/feature/2026-08-19-web-inline-mermaid-diagrams.md)。
...
- **Mermaid 图表是静态的**：内联表层将图表渲染为不可交互的 SVG（无平移/缩放，也无可点击节点）；交互式或 3D 可视化属于浏览器标签页 artifact，而非此表层。围栏仅在定稿一遍上渲染（流式围栏显示为纯代码块），无效的图表语法回退为源代码块。
