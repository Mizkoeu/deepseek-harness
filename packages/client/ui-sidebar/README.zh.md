...
侧边栏外壳插件：负责字标、New Session 操作、布局持有的折叠控件、可感知滚动的区域 seat，以及固定在底部的 Settings seat。[ui-workspace](../ui-workspace/README.md) 持有渲染到 `sidebar.workspaces` 的 Workspace 与 Session 浏览器；本包既不派生其中的行，也不持有其视图偏好。在普通宽度下，折叠到布局拥有的 56px 轨道仍属于本地呈现行为；低于 560px 时，完成收起的外壳只在布局的 44×52px 浮动表面中保留现有切换控件，打开后则以覆盖层恢复完整外壳。约定：[slot 系统标准](../../../.agents/notes/implemented/architecture/2026-07-22-slot-type-chain-implementation.md)。
...
实时收起时，外壳会把展开内容固定在当前宽度，并用 150ms 将其淡出。在普通宽度下，随后上方四个控件——外壳的侧栏切换与新建会话，以及通过 `sidebar.workspaces` 渲染的添加和搜索——共用一次 150ms 的淡入和 49px 左移，在布局的 300ms 栏滑动结束时一起进入 56px 轨道；每个 36px 控件盒都会沿同一条路径到达轨道左侧 10px 的内边距。在手机宽度的 viewport 上，完成收起后只保留外壳切换控件；打开覆盖层时，新建会话、Workspace 区域和页脚会重新出现。固定在底部的 `sidebar.settings` 控件只共用普通宽度下的淡入时序，不发生横向位移。页面初始即为收起状态时会静态渲染紧凑状态；减少动态效果模式会禁用两段过渡。
