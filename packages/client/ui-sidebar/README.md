...
During a live collapse, the shell holds the expanded content at its current width while it fades out for 150ms. At ordinary widths, the four upper controls—the shell toggle and New Session plus add and search rendered through `sidebar.workspaces`—then share one 150ms fade and 49px leftward translation into the 56px rail, ending with the layout's 300ms column slide; every 36px control box follows the same path to the rail's 10px left inset. On a phone-sized viewport, only the shell toggle remains after settlement; New Session, the workspace region, and the footer return when the overlay opens. The bottom-pinned `sidebar.settings` control shares the ordinary-width fade timing but has no horizontal translation. A page that starts collapsed renders its compact state statically, and reduced-motion mode disables both transitions.
