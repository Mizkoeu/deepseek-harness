...
Configure credentials, the model catalog, and deployment-specific transport settings per provider, keyed by the provider route itself. `apiKeyEnv` is a credential *reference* resolved per request, so no secret enters this file. Omitting it lets an installed catalog route use a stored pi-ai OAuth credential or provider-native ambient discovery; a configured reference that resolves to nothing fails the request with `MISSING_CREDENTIAL` instead, because falling through would authenticate with a different credential. One credential serves every model on its route.
...
The adapter stores structured pi-ai credentials in owner-only `$DSH_HOME/.pi-ai-credentials.json`. Run `dsh auth login github-copilot`, authorize the printed device code, then add GitHub Copilot from **Settings → Models** without entering an API key. A keyless profile lets pi-ai use the stored OAuth record and refresh its short-lived Copilot token under the store's cross-process writer lock. `dsh auth status github-copilot` reports whether the record exists without refreshing or exposing it; `dsh auth logout github-copilot` removes it. GitHub Enterprise login adds `--enterprise-domain <domain>`.
...
An explicit `apiKeyEnv` remains the highest-priority request credential. Use it for providers whose configured value is directly accepted by their model endpoint; do not set it on a GitHub Copilot OAuth profile, because the direct-key override bypasses pi-ai's stored token exchange and refresh.
...
Each collection shares the package's file-backed pi-ai credential store. The harness still resolves a named key through its own credential service before the request reaches pi-ai and passes it as the highest-priority `apiKey` override, preserving fail-loud reference semantics. A route naming no key lets pi-ai resolve stored OAuth or provider-native ambient credentials. OAuth refresh replaces the complete provider record atomically, while model collections remain immutable snapshots of routes and models rather than credentials.
