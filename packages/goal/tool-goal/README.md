...
- `update_goal(goal_id, revision, action, objective?, max_goal_rounds?, blocked_reason?)` supports `edit`, `pause`, `resume`, `complete`, and `blocked`. `objective` and `max_goal_rounds` apply only under `edit` (where an empty string or zero counts as omitted); every other action ignores them whatever their value, so a strict-mode endpoint that fills these optional fields cannot derail a pause, resume, complete, or block. `blocked_reason` is required only for `blocked`, is rejected on any other action, and is persisted with the stable code `model-reported`.
