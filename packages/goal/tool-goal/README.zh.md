...
- `update_goal(goal_id, revision, action, objective?, max_goal_rounds?, blocked_reason?)` 支持 `edit`、`pause`、`resume`、`complete` 和 `blocked`。`objective` 与 `max_goal_rounds` 仅在 `edit` 下生效（其中空字符串或零视为省略）；其余任何 action 都会忽略它们，无论取值如何，因此严格模式端点为这些可选字段填充的占位值不会干扰 pause、resume、complete 或 block。`blocked_reason` 只有在 action 为 `blocked` 时才必填，在其他 action 上会被拒绝，并以稳定代码 `model-reported` 持久化。
